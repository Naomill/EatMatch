...
const NavbarLight = () => {
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 py-5 px-4 md:px-[120px] transition-all duration-300 ${isScrolled ? 'bg-black/30 backdrop-blur' : 'bg-transparent'
        }`}
    >
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex justify-between items-center h-16">
          <Link to="/" className="flex items-center">
            <img src='/assets/Logo-dark.svg' alt='EatMatch logo' className='h-10 w-auto' />
          </Link>
